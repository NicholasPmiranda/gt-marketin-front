...
import { buscarDashLinhasConcluidasPorPeriodo } from "@/lib/dash-api"
import type { DashFiltros, DashLinhaTotal } from "@/types/dash"

const chartConfig = { total: { label: "Concluidas", color: "var(--primary)" } } satisfies ChartConfig

export function DashboardLinhasConcluidasPorPeriodo({ filtros }: { filtros: DashFiltros }) {
  ...
  useEffect(() => {
    async function carregar() {
      try {
        setIsLoading(true)
        setDados(await buscarDashLinhasConcluidasPorPeriodo(filtros))
      } catch {
        toast.error("Nao foi possivel carregar tarefas concluidas por periodo.")
      } finally {
        setIsLoading(false)
      }
    }
  ...
  return (
    <Card>
      <CardHeader><CardTitle>Concluidas por periodo</CardTitle></CardHeader>
      <CardContent>
